class JMount{
constructor(data={}){overJson(data,this)}
mount_(){['_init','_appends','_events','_limits','_requests'].forEach(mountMethod=>this[mountMethod]?.())}}
...
this._updated=true
this._sourceIds=arrayInSpread(sourceIds)
...
addSourceKeys_=(...keys)=>{
this._sourceKeys=this._sourceKeys.concat(arrayInSpread(keys))
return this.osk_()}
ask_=this.addSourceKeys_
overSourceKeys_=(...k)=>{
k=arrayInSpread(k)
return overJson(this,{},k.length?k:this._sourceKeys)
}
osk_=this.overSourceKeys_
overSourceKeysPlus_=(...k)=>{
k=this._sourceKeys.concat(arrayInSpread(k))
return this.osk_(k)
}
psk_=this.overSourceKeysPlus_
innerAnchor_=(e,k)=>{
this[k].id=this._prefix+k+ID_MAKER.next().value
e.href='#'+this[k].id
};
defClasses_=(...k)=>arrayInSpread(k)
.reduce((k,m)=>k.concat(this._prefix+(/^\$/.test(m)?m.substring(1):m)),[])
insertClasses_=(e,...k)=>addClasses(e,this.defClasses_(k))
removeClasses_=(e,...classes)=>removeClasses(e,this.defClasses_(classes))
viewValues_=(fromUpdate,...addVarKeys)=>{
let regInput=/^\$input(.)(.*)/,
regUpdate=/^\$inputUpdate(.)(.*)/,
index=fromUpdate?12:6,
strike=arrayInSpread(addVarKeys).reduce((json,key)=>{json[key]=this[key]; return json},{}),
...
super({notIncludeDivs: arrayInSpread(notIncludeDivs)}).mount_()}
_init(){
let elementOptions=['menu','navigation','content','footer'];
elementOptions.filter(eOption=>!(this.notIncludeDivs.includes(eOption)))
.map(eOption=>A.div(eOption))
.forEach(aElement=>document.body.a(aElement))
}
...
constructor(urlBase){this.urlBase=urlBase.replace(/([^\/])\/+$/,'$1')}
...
addHeaders(...headers){
let newHeaders=arrayInSpread(headers).map(header=>{
...
asArray(errors).filter(validation=>validation.input)
.forEach(validation=>{
...
_loadData(columns){
columns.length&&this.setHeadRowFromItems(asArray(columns))
}
...
function arrayInSpread(spreadItens,exaustive=true){
while(spreadItens.length==1&&Array.isArray(spreadItens[0])){
if(!exaustive)return spreadItens[0]
spreadItens=spreadItens[0]
}
return spreadItens
}
function asArray(...objects){return arrayInSpread(objects,false)}
function overJson(jsonSource,jsonDest,...keys){
keys=arrayInSpread(keys)
...
const aMode=(tag,varName,tagView)=>{
...
arrayInSpread(elements).forEach(e=>viewOrNode(parent).removeChild(viewOrNode(e)))
}
...
arrayInSpread(nodes).forEach(node=>parentNode.insertBefore(viewOrNode(node),referenceNode.nextSibling))
}
function insertBefore(referenceNode,...nodes){
let parentNode=viewOrNode(referenceNode).parentNode;
arrayInSpread(nodes).forEach(node=>parentNode.insertBefore(viewOrNode(node),referenceNode))
}
...
}else{appendTo(element,element.textNode)}}
...
arrayInSpread(elements).forEach(e=>viewOrNode(parent).appendChild(viewOrNode(e)))
return parent
}
...
element=viewOrNode(element)
element&&element.classList[action](...arrayInSpread(classes))
return element
}
...
arrayInSpread(textareas).forEach($input=>{
$input.rows=2;
